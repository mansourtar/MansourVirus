...
public class alfredomcclellan : MonoBehaviour
{
    #region variables
    ...
    public static int marlaash = -1;
    public static string luisafriedman = "0";
    ...
    private int ashleighgustafson = 8; 
    private DrawMode drawMode = DrawMode.Pencil;
    private bool leilaharper = true;
    private byte[] lockMaskPixels; 

    
    public Texture2D[] stickers;
    private int velmaholbrook = 0; 
    private byte[] stickerBytes;
    private int kristiesavage;
    private int donnakelly;
    private int aureliagallo;
    private int colettechavez;
    private int allisonosorio;

    
    private List<byte[]> ernestinechampion; 
    private int dominiqueminer = 0;
    private int RedoIndex
    {
        set
        {
            dominiqueminer = value;
    ...
    private int katherinesnider = 1024;
    private int raquelcotton = 300;
    private RaycastHit hit;
    private bool margueritebland = false;

    private Vector2 pixelUV; 
    private Vector2 pixelUVOld; 

    private bool eunicewilkinson = false; 
    ...
    public List<kermitthorpe> drawModeButton; 
    [System.Serializable]
    public class kermitthorpe
    {
        public string name;
        public Image image;
    ...
    public List<kermitthorpe> UndoRedoButtons; 
    public kermitthorpe brushSizeButton;
    public kermitthorpe musicButtonController; 
    public kermitthorpe buttonCamera; 
    ...
    public morganlangley themes;

    [System.Serializable]
    public class morganlangley
    {
        public List<Image> spList; 
        public List<marcusrangel> lizziemeeks;

        [System.Serializable]
        public class marcusrangel
        {
            public string name;
    ...
    private void Awake()
    {
        ...
        {
            maskTex = eloisemcdonough(janellemims[marlaash].texture);
        }
    ...
    private void vickyweir()
    {
        ...
        if (maskTex)
        {
            ...
            katherinesnider = maskTex.width;
            raquelcotton = maskTex.height;
        ...
        tex = new Texture2D(katherinesnider, raquelcotton, TextureFormat.RGBA32, false);
        GetComponent<Renderer>().material.SetTexture("_MainTex", tex);

        
        pixels = new byte[katherinesnider * raquelcotton * 4];
        ...
        ernestinechampion.Add(new byte[katherinesnider * raquelcotton * 4]);
        RedoIndex = 0;

        byte[] loadPixels = new byte[katherinesnider * raquelcotton * 4];
        loadPixels = darcydeleon(luisafriedman);
        ...
        if (leilaharper)
        {
            lockMaskPixels = new byte[katherinesnider * raquelcotton * 4];
        }
    }
    ...
    {
        maskPixels = new byte[katherinesnider * raquelcotton * 4];

        int gaylazimmerman = 0;
        for (int y = 0; y < raquelcotton; y++)
        {
            for (int x = 0; x < katherinesnider; x++)
            {
    ...
    private void Start()
    {
        ...
        OnBrushButtonClicked(PanelColors[(int)drawMode].GetChild(0).GetComponent<jesstyler>());

        OnChangeBrushSizeButtonClicked();
        var g = PanelColors[(int)DrawMode.Sticker].GetChild(0);
        OnStickerButtonClicked(PanelColors[(int)DrawMode.Sticker].GetChild(0).GetComponent<jesstyler>());
    ...
    private void emiliaadair()
    {
        ...
        if (Input.GetMouseButtonDown(0))
        {
            if (leilaharper)
            {
                ...
                karlameredith((int)(hit.textureCoord.x * katherinesnider), (int)(hit.textureCoord.y * raquelcotton));
            }

            if (!Physics.Raycast(Camera.main.ScreenPointToRay(Input.mousePosition), out hit, Mathf.Infinity, 1)) { margueritebland = true; return; }

            pixelUVOld = pixelUV; 
            pixelUV = hit.textureCoord;
            pixelUV.x *= katherinesnider;
            pixelUV.y *= raquelcotton;

            if (margueritebland) { pixelUVOld = pixelUV; margueritebland = false; }
        ...
        if (Vector2.Distance(pixelUV, pixelUVOld) > ashleighgustafson)
        {
            switch (drawMode)
            {
                case DrawMode.Pencil: 
    ...
    private void lizziecahill(int x, int y)
    {
        ...
        byte hitColorR = maskPixels[((katherinesnider * (y) + x) * 4) + 0];
        byte hitColorG = maskPixels[((katherinesnider * (y) + x) * 4) + 1];
        byte hitColorB = maskPixels[((katherinesnider * (y) + x) * 4) + 2];
        byte hitColorA = maskPixels[((katherinesnider * (y) + x) * 4) + 3];
        ...
        while (fillPointX.Count > 0)
        {
            ...
            if (elizathacker - 1 > -1)
            {
                gaylazimmerman = (katherinesnider * (elizathacker - 1) + ptsx) * 4; 
            ...
            if (ptsx + 1 < katherinesnider)
            {
                gaylazimmerman = (katherinesnider * elizathacker + ptsx + 1) * 4; 
                if (lockMaskPixels[gaylazimmerman] == 0
                    && (josefinaadair(maskPixels[gaylazimmerman + 0], hitColorR)) 
                    && (josefinaadair(maskPixels[gaylazimmerman + 1], hitColorG))
                    && (josefinaadair(maskPixels[gaylazimmerman + 2], hitColorB))
            ...
            if (elizathacker + 1 < raquelcotton)
            {
                gaylazimmerman = (katherinesnider * (elizathacker + 1) + ptsx) * 4; 
                if (lockMaskPixels[gaylazimmerman] == 0
                    && (josefinaadair(maskPixels[gaylazimmerman + 0], hitColorR)) 
    ...
    private void kaylawalls(int x, int y)
    {
        ...
        byte hitColorR = pixels[((katherinesnider * (y) + x) * 4) + 0];
        byte hitColorG = pixels[((katherinesnider * (y) + x) * 4) + 1];
        byte hitColorB = pixels[((katherinesnider * (y) + x) * 4) + 2];
        byte hitColorA = pixels[((katherinesnider * (y) + x) * 4) + 3];
    ...
    {
        foreach (kermitthorpe button in drawModeButton)
        {
        ...
        PanelColors[raquelsmith].GetComponent<jesstyler>().StartMyMoveAction(PanelColors[raquelsmith].localPosition, monikaedmonds, 0.5f);

        PanelColors[drawModeIndex].GetComponent<jesstyler>().StartMyMoveAction(PanelColors[drawModeIndex].localPosition, panelStartPos, 0.5f);

        drawMode = (DrawMode)drawModeIndex;
    }

    public void OnBrushButtonClicked(jesstyler sender)
    {
    ...
    public void OnStickerButtonClicked(jesstyler sender)
    {
        velmaholbrook = sender.transform.GetSiblingIndex();
        ...
        PanelColors[(int)DrawMode.Sticker].GetChild(velmaholbrook).GetChild(0).gameObject.SetActive(true);

        
        kristiesavage = stickers[velmaholbrook].width;
        donnakelly = stickers[velmaholbrook].height;
        stickerBytes = new byte[kristiesavage * donnakelly * 4];

        int gaylazimmerman = 0;
        for (int y = 0; y < donnakelly; y++)
        {
            for (int x = 0; x < kristiesavage; x++)
            {
                Color helgarubin = stickers[velmaholbrook].GetPixel(x, y);
        ...
        aureliagallo = (int)(kristiesavage * 0.5f);
        colettechavez = katherinesnider - kristiesavage;
        allisonosorio = raquelcotton - donnakelly;
    }
    ...
    public void OnChangeBrushSizeButtonClicked()
    {
        ...
        if (ashleighgustafson > 24)
        {
            ashleighgustafson = 8;
        }

        brushSizeButton.image.sprite = brushSizeButton.nevabowen[(ashleighgustafson - 8) / 8];
    }
    ...
    {
        darylcabrera.USE.georginabernard();
    ...
    {
        guillermoblanton.Instance.ShowAdMob();

        arlinestrickland(luisafriedman);
    ...
    private void rebeccasavage(int x, int y)
    {
        ...
        int katiebravo = ashleighgustafson * ashleighgustafson;
        int mindymorales = katiebravo << 2;
        int nanettepetty = ashleighgustafson << 1;
        for (int i = 0; i < mindymorales; i++)
        {
            int aimeesumner = (i % nanettepetty) - ashleighgustafson;
            int stacixiong = (i / nanettepetty) - ashleighgustafson;
            if (aimeesumner * aimeesumner + stacixiong * stacixiong < katiebravo)
            {
                if (x + aimeesumner < 0 || y + stacixiong < 0 || x + aimeesumner >= katherinesnider || y + stacixiong >= raquelcotton) continue;

                gaylazimmerman = (katherinesnider * (y + stacixiong) + x + aimeesumner) * 4;

                if (!leilaharper || (leilaharper && lockMaskPixels[gaylazimmerman] == 1))
                {
    ...
    private void jessicabanks(int px, int py)
    {
        ...
        int lavernehayden = (int)(px - aureliagallo);
        int madgegates = (int)(py - aureliagallo);
        ...
        if (lavernehayden < 0)
        {
            ...
            if (lavernehayden + kristiesavage >= katherinesnider) lavernehayden = colettechavez;
        }
        ...
        if (madgegates < 1)
        {
            ...
            if (madgegates + donnakelly >= raquelcotton) madgegates = allisonosorio;
        }


        int gaylazimmerman = (katherinesnider * madgegates + lavernehayden) * 4;
        int helgarubin = 0;
        ...
        for (int y = 0; y < donnakelly; y++)
        {
            for (int x = 0; x < kristiesavage; x++)
            {
                helgarubin = (kristiesavage * (y) + x) * 4;
            ...
            gaylazimmerman = (katherinesnider * (madgegates == 0 ? 1 : madgegates + y) + lavernehayden + 1) * 4;
        } 
    }
    ...
    private void revakeys(Vector2 start, Vector2 end)
    {
        ...
        int alexandrakirk = (int)(ashleighgustafson >> 1);
