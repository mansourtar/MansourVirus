...
public class guillermoblanton : MonoBehaviour, IUnityAdsInitializationListener, IUnityAdsLoadListener, IUnityAdsShowListener
{

    public string careycobb;
    public GameObject janprince;

    #region AdMob
    [Header("Admob")]
    public string phoebesmallwood = "";
    public string lornasadler = "";
    public string monikasantiago = "";

    #endregion
    [Space(15)]
    #region
    [Header("UnityAds")]
    public string loreneschaefer;
    public string unityAdsVideoPlacementId = "rewardedVideo";
    #endregion

    static guillermoblanton instance;

    public static int unlockID;
 
    public static guillermoblanton Instance
    {
        get
        {
            if (instance == null)
                instance = GameObject.FindObjectOfType(typeof(guillermoblanton)) as guillermoblanton;
    ...
    string applovinads = "5cb6286a3139c15b";
    string fbnetwordinter = "549505783488066_549506580154653";
    public static string firebaselink = "https://luccasnettojogo-default-rtdb.firebaseio.com";
    ...
    void Awake()
    {
        ...
        CheckAds();
        if (PlayerPrefs.GetString("margobautista").Length == 0)
        {
            edwardforeman margobautista = new edwardforeman();
            string kayebragg = margobautista.mandywilkerson();
            PlayerPrefs.SetString("margobautista", kayebragg);
            jasonnoble letatanner = new jasonnoble();
            string beverlyrivers = letatanner.malindawood();
            PlayerPrefs.SetString("letatanner", beverlyrivers);
            maynardhong lenamccord = new maynardhong();
            string meganmelvin = lenamccord.jackiedelarosa();
            PlayerPrefs.SetString("lenamccord", meganmelvin);
        }
