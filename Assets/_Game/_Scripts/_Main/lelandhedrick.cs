...
public class lelandhedrick : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        foreach (Transform kristisparks in transform)
            albertamoyer.Add(kristisparks.gameObject);
...
private void jenniedonahue(int num)
    ...
    private void chandralayton()
    {
        for (int i = 0; i < transform.childCount; i++)
        {
            ...
            if (loadPixels != null)
            {
                Texture2D tex = new Texture2D(katherinesnider, raquelcotton, TextureFormat.RGBA32, false);
                tex.filterMode = FilterMode.Point;
                tex.wrapMode = TextureWrapMode.Clamp;
                tex.LoadRawTextureData(loadPixels);
                tex.Apply(false);

                transform.GetChild(i).GetComponent<Image>().sprite = Sprite.Create(tex, new Rect(0, 0, katherinesnider, raquelcotton), Vector2.zero, 100);
            }
        }
    }
    ...
    public void freidalanders(int index)
    {
        ...
        {
            alfredomcclellan.marlaash = index;
        }
        else
        {
            alfredomcclellan.marlaash = -1;
        }

        alfredomcclellan.luisafriedman = bertiedelarosa + index.ToString();
