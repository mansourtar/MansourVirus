...
public class vincentdelatorre : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        foreach (Transform lesleyguzman in transform)
            albertamoyer.Add(lesleyguzman.gameObject);
...
private void jenniedonahue(int num)
    ...
    private void chandralayton()
    {
        for (int i = 0; i < transform.childCount; i++)
        {
            ...
            if (loadPixels != null)
            {
                Texture2D tex = new Texture2D(maystafford, milliehouse, TextureFormat.RGBA32, false);
                tex.filterMode = FilterMode.Point;
                tex.wrapMode = TextureWrapMode.Clamp;
                tex.LoadRawTextureData(loadPixels);
                tex.Apply(false);

                transform.GetChild(i).GetComponent<Image>().sprite = Sprite.Create(tex, new Rect(0, 0, maystafford, milliehouse), Vector2.zero, 100);
            }
        }
    }
    ...
    public void freidalanders(int index)
    {
        ...
        {
            arnoldhood.sophiedunlap = index;
        }
        else
        {
            arnoldhood.sophiedunlap = -1;
        }

        arnoldhood.anitabloom = bertiedelarosa + index.ToString();
