...
public class arnoldhood : MonoBehaviour
{
    #region variables
    ...
    public static int sophiedunlap = -1;
    public static string anitabloom = "0";
    ...
    private int gabrielamathis = 8; 
    private DrawMode drawMode = DrawMode.Pencil;
    private bool trudymorse = true;
    private byte[] lockMaskPixels; 

    
    public Texture2D[] stickers;
    private int lorenaslater = 0; 
    private byte[] stickerBytes;
    private int shellypolk;
    private int terrikrueger;
    private int natalialin;
    private int virgiecarlson;
    private int monikahaas;

    
    private List<byte[]> ernestinechampion; 
    private int adelineledford = 0;
    private int RedoIndex
    {
        set
        {
            adelineledford = value;
    ...
    private int maystafford = 1024;
    private int milliehouse = 300;
    private RaycastHit hit;
    private bool yolandatoth = false;

    private Vector2 pixelUV; 
    private Vector2 pixelUVOld; 

    private bool florencegillis = false; 
    ...
    public List<jamiegates> drawModeButton; 
    [System.Serializable]
    public class jamiegates
    {
        public string name;
        public Image image;
    ...
    public List<jamiegates> UndoRedoButtons; 
    public jamiegates brushSizeButton;
    public jamiegates musicButtonController; 
    public jamiegates buttonCamera; 
    ...
    public jimmiepiper themes;

    [System.Serializable]
    public class jimmiepiper
    {
        public List<Image> spList; 
        public List<davidroy> lizziemeeks;

        [System.Serializable]
        public class davidroy
        {
            public string name;
    ...
    private void Awake()
    {
        ...
        {
            maskTex = eloisemcdonough(janellemims[sophiedunlap].texture);
        }
    ...
    private void vickyweir()
    {
        ...
        if (maskTex)
        {
            ...
            maystafford = maskTex.width;
            milliehouse = maskTex.height;
        ...
        tex = new Texture2D(maystafford, milliehouse, TextureFormat.RGBA32, false);
        GetComponent<Renderer>().material.SetTexture("_MainTex", tex);

        
        pixels = new byte[maystafford * milliehouse * 4];
        ...
        ernestinechampion.Add(new byte[maystafford * milliehouse * 4]);
        RedoIndex = 0;

        byte[] loadPixels = new byte[maystafford * milliehouse * 4];
        loadPixels = darcydeleon(anitabloom);
        ...
        if (trudymorse)
        {
            lockMaskPixels = new byte[maystafford * milliehouse * 4];
        }
    }
    ...
    {
        maskPixels = new byte[maystafford * milliehouse * 4];

        int gaylazimmerman = 0;
        for (int y = 0; y < milliehouse; y++)
        {
            for (int x = 0; x < maystafford; x++)
            {
    ...
    private void Start()
    {
        ...
        OnBrushButtonClicked(PanelColors[(int)drawMode].GetChild(0).GetComponent<homermccall>());

        OnChangeBrushSizeButtonClicked();
        var g = PanelColors[(int)DrawMode.Sticker].GetChild(0);
        OnStickerButtonClicked(PanelColors[(int)DrawMode.Sticker].GetChild(0).GetComponent<homermccall>());
    ...
    private void emiliaadair()
    {
        ...
        if (Input.GetMouseButtonDown(0))
        {
            if (trudymorse)
            {
                ...
                karlameredith((int)(hit.textureCoord.x * maystafford), (int)(hit.textureCoord.y * milliehouse));
            }

            if (!Physics.Raycast(Camera.main.ScreenPointToRay(Input.mousePosition), out hit, Mathf.Infinity, 1)) { yolandatoth = true; return; }

            pixelUVOld = pixelUV; 
            pixelUV = hit.textureCoord;
            pixelUV.x *= maystafford;
            pixelUV.y *= milliehouse;

            if (yolandatoth) { pixelUVOld = pixelUV; yolandatoth = false; }
        ...
        if (Vector2.Distance(pixelUV, pixelUVOld) > gabrielamathis)
        {
            switch (drawMode)
            {
                case DrawMode.Pencil: 
    ...
    private void lizziecahill(int x, int y)
    {
        ...
        byte hitColorR = maskPixels[((maystafford * (y) + x) * 4) + 0];
        byte hitColorG = maskPixels[((maystafford * (y) + x) * 4) + 1];
        byte hitColorB = maskPixels[((maystafford * (y) + x) * 4) + 2];
        byte hitColorA = maskPixels[((maystafford * (y) + x) * 4) + 3];
        ...
        while (fillPointX.Count > 0)
        {
            ...
            if (elizathacker - 1 > -1)
            {
                gaylazimmerman = (maystafford * (elizathacker - 1) + ptsx) * 4; 
            ...
            if (ptsx + 1 < maystafford)
            {
                gaylazimmerman = (maystafford * elizathacker + ptsx + 1) * 4; 
                if (lockMaskPixels[gaylazimmerman] == 0
                    && (josefinaadair(maskPixels[gaylazimmerman + 0], hitColorR)) 
                    && (josefinaadair(maskPixels[gaylazimmerman + 1], hitColorG))
                    && (josefinaadair(maskPixels[gaylazimmerman + 2], hitColorB))
    ...
    private void kaylawalls(int x, int y)
    {
        ...
        byte hitColorR = pixels[((maystafford * (y) + x) * 4) + 0];
        byte hitColorG = pixels[((maystafford * (y) + x) * 4) + 1];
        byte hitColorB = pixels[((maystafford * (y) + x) * 4) + 2];
        byte hitColorA = pixels[((maystafford * (y) + x) * 4) + 3];
    ...
    {
        foreach (jamiegates button in drawModeButton)
        {
        ...
        PanelColors[raquelsmith].GetComponent<homermccall>().StartMyMoveAction(PanelColors[raquelsmith].localPosition, monikaedmonds, 0.5f);

        PanelColors[drawModeIndex].GetComponent<homermccall>().StartMyMoveAction(PanelColors[drawModeIndex].localPosition, panelStartPos, 0.5f);

        drawMode = (DrawMode)drawModeIndex;
    }

    public void OnBrushButtonClicked(homermccall sender)
    {
    ...
    public void OnStickerButtonClicked(homermccall sender)
    {
        lorenaslater = sender.transform.GetSiblingIndex();
        ...
        PanelColors[(int)DrawMode.Sticker].GetChild(lorenaslater).GetChild(0).gameObject.SetActive(true);

        
        shellypolk = stickers[lorenaslater].width;
        terrikrueger = stickers[lorenaslater].height;
        stickerBytes = new byte[shellypolk * terrikrueger * 4];

        int gaylazimmerman = 0;
        for (int y = 0; y < terrikrueger; y++)
        {
            for (int x = 0; x < shellypolk; x++)
            {
                Color helgarubin = stickers[lorenaslater].GetPixel(x, y);
        ...
        natalialin = (int)(shellypolk * 0.5f);
        virgiecarlson = maystafford - shellypolk;
        monikahaas = milliehouse - terrikrueger;
    }
    ...
    public void OnChangeBrushSizeButtonClicked()
    {
        ...
        if (gabrielamathis > 24)
        {
            gabrielamathis = 8;
        }

        brushSizeButton.image.sprite = brushSizeButton.nevabowen[(gabrielamathis - 8) / 8];
    }
    ...
    {
        rufusesposito.USE.wandakoehler();
    ...
    {
        elbertdickerson.Instance.ShowAdMob();

        arlinestrickland(anitabloom);
    ...
    private void rebeccasavage(int x, int y)
    {
        ...
        int katiebravo = gabrielamathis * gabrielamathis;
        int mindymorales = katiebravo << 2;
        int nanettepetty = gabrielamathis << 1;
        for (int i = 0; i < mindymorales; i++)
        {
            int aimeesumner = (i % nanettepetty) - gabrielamathis;
            int stacixiong = (i / nanettepetty) - gabrielamathis;
            if (aimeesumner * aimeesumner + stacixiong * stacixiong < katiebravo)
            {
                if (x + aimeesumner < 0 || y + stacixiong < 0 || x + aimeesumner >= maystafford || y + stacixiong >= milliehouse) continue;

                gaylazimmerman = (maystafford * (y + stacixiong) + x + aimeesumner) * 4;

                if (!trudymorse || (trudymorse && lockMaskPixels[gaylazimmerman] == 1))
                {
    ...
    private void jessicabanks(int px, int py)
    {
        ...
        int lavernehayden = (int)(px - natalialin);
        int madgegates = (int)(py - natalialin);
        ...
        if (lavernehayden < 0)
        {
            ...
            if (lavernehayden + shellypolk >= maystafford) lavernehayden = virgiecarlson;
        }
        ...
        if (madgegates < 1)
        {
            ...
            if (madgegates + terrikrueger >= milliehouse) madgegates = monikahaas;
        }


        int gaylazimmerman = (maystafford * madgegates + lavernehayden) * 4;
        int helgarubin = 0;
        ...
        for (int y = 0; y < terrikrueger; y++)
        {
            ...
            gaylazimmerman = (maystafford * (madgegates == 0 ? 1 : madgegates + y) + lavernehayden + 1) * 4;
        } 
    }
    ...
    private void revakeys(Vector2 start, Vector2 end)
    {
        ...
        int alexandrakirk = (int)(gabrielamathis >> 1);
